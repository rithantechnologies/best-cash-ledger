...
type Tab="payables"|"receivables"|"settlements"|"cardClearing";
...
type DueRow={kind:Tab;dueAt:string|null;remainingAmount:number;item:Payable|Receivable|Settlement|CardClearing};
...
export default function DuesPage(){
 ...
 const openCardClearings=useMemo(()=>cardClearings.filter(x=>Number(x.principalRemaining)>0.001||Number(x.commissionRemaining)>0.001),[cardClearings]);
 const rows:DueRow[]=tab==="payables"
  ...
  :tab==="receivables"
   ?openReceivables.map(item=>({kind:"receivables",dueAt:item.dueAt,remainingAmount:Number(item.remainingAmount),item}))
   :tab==="settlements"
    ?openSettlements.map(item=>({kind:"settlements",dueAt:item.dueAt,remainingAmount:Number(item.remainingAmount),item}))
    :openCardClearings.map(item=>({kind:"cardClearing",dueAt:item.nextFollowUpAt,remainingAmount:Number(item.principalRemaining)+Number(item.commissionRemaining),item}));
 ...
 const fullListHref=tab==="payables"?"/payables":tab==="receivables"?"/receivables":tab==="settlements"?"/provider-settlements":"/transactions/card-due-clearing";
 const sectionName=tab==="payables"?"customer payables":tab==="receivables"?"receivables":tab==="settlements"?"provider clearing":"card due clearing";
 ...
 return <AppShell><PageFrame width="max-w-5xl">
  ...
  <SegmentedTabs<Tab> value={tab} onChange={v=>{setTab(v);setFilter("all");}} items={[
   ...
   {value:"cardClearing",label:"Card clearing",count:openCardClearings.length},
  ]}/>
  ...
  <Surface className="overflow-hidden">
   {filtered.length?<div className="divide-y divide-[var(--border)]">{filtered.map(row=>{
    ...
    if(row.kind==="cardClearing"){const c=row.item as CardClearing;const principal=Number(c.principalRemaining),commission=Number(c.commissionRemaining);return <Link key={c.id} href={"/transactions/card-due-clearing?id="+c.transactionId} className="flex items-center justify-between gap-3 px-4 py-3.5 hover:bg-[var(--surface-soft)]"><div className="min-w-0"><p className="truncate text-sm font-semibold">{c.transaction.customer?.fullName??"Customer"} · {c.transaction.transactionNumber}</p><p className="mt-0.5 truncate text-xs text-[var(--text-muted)]">{c.customerCard.bankName} •••• {c.customerCard.lastFourDigits} · principal {money(principal)} · commission {money(commission)}</p><p className={"mt-0.5 text-xs "+(late?"text-rose-600":"text-[var(--text-muted)]")}>{relative(c.nextFollowUpAt)}</p></div><div className="text-right"><p className="money font-semibold">{money(principal+commission)}</p><span className="text-xs font-semibold text-[var(--accent)]">Collect →</span></div></Link>}
