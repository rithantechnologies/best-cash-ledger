...
export default function CardDueClearingPage(){
 ...
 return <AppShell><PageFrame width="max-w-7xl">
  <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
   <div><p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[var(--text-muted)]">Transactions</p><h1 className="mt-1 text-2xl font-black tracking-[-.035em]">Card Due Clearing</h1><p className="mt-1 max-w-3xl text-sm text-[var(--text-muted)]">Pay a customer card due from a client account, recover the principal after the limit returns, and collect the 3% service charge now or later.</p></div>
   <Link href="/transactions" className="text-xs font-bold text-[var(--accent)]">← Transactions</Link>
  </div>

  <div className="grid grid-cols-3 gap-2 text-xs">
   <div className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 font-bold text-rose-700">Red · no recovery yet</div>
   <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 font-bold text-amber-700">Orange · partly settled</div>
   <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 font-bold text-emerald-700">Green · everything collected</div>
  </div>
  ...
  <Surface className="overflow-visible">
   <div className="border-b border-[var(--border)] px-4 py-3.5 sm:px-5"><h2 className="text-sm font-black">New due clearing</h2><p className="mt-0.5 text-[11px] text-[var(--text-muted)]">Default commission is 3%. You can finish everything now or leave any balance open.</p></div>
   <form onSubmit={createCase} className="space-y-4 p-4 sm:p-5">
    ...
    <div className="grid gap-3 lg:grid-cols-2">
     <div className="rounded-2xl border border-[var(--border)] p-3.5">
      <label className="flex cursor-pointer items-center gap-2 text-sm font-bold"><input type="checkbox" checked={recoverNow} onChange={e=>{setRecoverNow(e.target.checked);if(e.target.checked)setRecoveryAmount(String(num(dueAmount)));}}/> Recover principal now</label>
     ...
     <div className="rounded-2xl border border-[var(--border)] p-3.5">
      <label className="flex cursor-pointer items-center gap-2 text-sm font-bold"><input type="checkbox" checked={collectNow} onChange={e=>{setCollectNow(e.target.checked);if(e.target.checked)setCollectionAmount(String(commission));}}/> Collect commission now</label>
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-bold text-[var(--text-muted)]">Due payment reference</span><input className="app-control" value={reference} onChange={e=>setReference(e.target.value)} placeholder="UTR / PhonePe reference"/></label><label><span className="mb-1.5 block text-xs font-bold text-[var(--text-muted)]">Notes</span><input className="app-control" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Optional note"/></label></div>
    <div className="flex items-center justify-between gap-3 border-t border-[var(--border)] pt-4"><div className="text-xs text-[var(--text-muted)]">Total customer obligation: <b className="money text-[var(--text)]">{money(num(dueAmount)+commission)}</b></div><button disabled={saving} className="app-primary-button min-h-11 px-5 text-sm font-black disabled:opacity-50">{saving?"Saving…":"Save due clearing"}</button></div>
   </form>
  </Surface>
