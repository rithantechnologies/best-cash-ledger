 ...
 return <AppShell><PageFrame className="ui-preview transactions-preview">
  ...
  {loading?<PageLoader label="Loading transactions…"/>:<>
   <div className="space-y-4 md:hidden">
    {[...grouped.entries()].map(([day,rows])=><section key={day}><div className="sticky top-15 z-10 mb-1.5 bg-[var(--bg)] py-1 text-xs font-semibold text-[var(--text-muted)]">{day}</div><Surface className="overflow-hidden"><div className="divide-y divide-[var(--border)]">{rows.map(tx=>{const fees=sum(tx.charges),earn=sum(tx.commissions);return <Link key={tx.id} href={"/transactions/"+tx.id} className="block px-4 py-3.5 hover:bg-[var(--surface-soft)]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{label(tx.transactionType)}{tx.customer?" · "+tx.customer.fullName:""}</p><p className="mt-0.5 truncate text-xs text-[var(--text-muted)]">{tx.transactionNumber} · {new Date(tx.transactionAt).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})}</p></div><p className="money shrink-0 text-sm font-bold">{money(tx.grossAmount)}</p></div><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px]"><span className="text-[var(--text-muted)]">Customer / net <b className="text-[var(--text)]">{money(tx.netAmount??tx.grossAmount)}</b></span>{fees>0?<span className="text-[var(--money-out)]">Fees −{money(fees)}</span>:null}{earn>0?<span className="font-bold text-[var(--money-in)]">Earned +{money(earn)}</span>:null}<StatusBadge tone={statusTone(tx.status) as "slate"|"emerald"|"amber"|"rose"}>{label(tx.status)}</StatusBadge></div></Link>})}</div></Surface></section>)}
    {!items.length?<EmptyState title="No transactions" description="Try another date or filter."/>:null}
   </div>

   <Surface className="hidden overflow-hidden md:block"><div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-sm"><thead className="bg-[var(--surface-soft)] text-left text-[11px] text-[var(--text-muted)]"><tr><th className="px-4 py-3">Date</th><th>Service</th><th>Customer</th><th>Transaction</th><th className="text-right">Processed</th><th className="text-right">Fees</th><th className="text-right">Earnings</th><th className="text-right">Customer / net</th><th className="pl-4">Status</th></tr></thead><tbody>{items.map(tx=>{const fees=sum(tx.charges),earn=sum(tx.commissions);return <tr key={tx.id} className="border-t border-[var(--border)] hover:bg-[var(--surface-soft)]"><td className="px-4 py-3 text-xs">{new Date(tx.transactionAt).toLocaleString("en-IN")}</td><td className="font-medium">{label(tx.transactionType)}</td><td>{tx.customer?.fullName??"—"}</td><td><Link className="font-semibold text-[var(--accent)]" href={"/transactions/"+tx.id}>{tx.transactionNumber}</Link></td><td className="money text-right">{money(tx.grossAmount)}</td><td className="money text-right text-[var(--money-out)]">{fees?money(fees):"—"}</td><td className="money text-right font-bold text-[var(--money-in)]">{earn?money(earn):"—"}</td><td className="money text-right font-semibold">{money(tx.netAmount??tx.grossAmount)}</td><td className="pl-4"><StatusBadge tone={statusTone(tx.status) as "slate"|"emerald"|"amber"|"rose"}>{label(tx.status)}</StatusBadge></td></tr>})}</tbody></table></div></Surface>
