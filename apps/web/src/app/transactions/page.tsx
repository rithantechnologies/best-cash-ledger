...
function payoutStatus(tx:Tx){
 if(tx.transactionType!=="CARD_SWIPE"||!tx.payable)return null;
 const p=tx.payable;
 if(p.status==="PAID")return {label:"Payout Paid",tone:"emerald" as const};
 if(p.status==="PARTIALLY_PAID")return {label:"Payout Partially Paid",tone:"amber" as const};
 if(["CANCELLED","REVERSED"].includes(p.status))return {label:"Payout "+p.status.replaceAll("_"," "),tone:"rose" as const};
...
export default function TransactionsPage(){
 ...
 return <AppShell><PageFrame className="ui-preview transactions-preview">
  ...
  <Surface className="grid gap-2 p-3 sm:grid-cols-4">
   ...
   <select className="app-control" value={status} onChange={e=>setStatus(e.target.value)}><option value="">All status</option>{["PENDING","PARTIALLY_PAID","COMPLETED","CANCELLED","REVERSED"].map(x=><option key={x}>{x}</option>)}</select>
  </Surface>
