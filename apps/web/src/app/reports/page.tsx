...
export default function ReportsPage(){
 ...
 const totals=useMemo(()=>transactions.reduce((a,t)=>({gross:a.gross+Number(t.grossAmount),commission:a.commission+sum(t.commissions),charges:a.charges+sum(t.charges)}),{gross:0,commission:0,charges:0}),[transactions]);
 ...
 function downloadCsv(){
  ...
  if(tab==="earnings"){rows=[["Transaction","Date","Service","Customer","Processed","Commission","External Fees","Status"],...transactions.map(t=>[t.transactionNumber,t.transactionAt,serviceName(t.transactionType),t.customer?.fullName??"",t.grossAmount,sum(t.commissions),sum(t.charges),t.status])];name="cash-ledger-earnings.csv";}
 ...
 return <AppShell><PageFrame>
  ...
  {tab==="earnings"?<>
   <Surface className="overflow-hidden">
    ...
    <div className="grid grid-cols-2 gap-px bg-[var(--border)] lg:grid-cols-4"><div className="bg-[var(--surface)] p-4"><p className="text-xs text-[var(--text-muted)]">Processed volume</p><p className="money mt-1 text-xl font-black">{money(totals.gross)}</p></div><div className="bg-[var(--surface)] p-4"><p className="text-xs text-[var(--text-muted)]">Business commission</p><p className="money mt-1 text-xl font-black text-[var(--money-in)]">{money(totals.commission)}</p></div><div className="bg-[var(--surface)] p-4"><p className="text-xs text-[var(--text-muted)]">Provider / bank fees</p><p className="money mt-1 text-xl font-black text-[var(--money-out)]">{money(totals.charges)}</p></div><div className="bg-[var(--surface)] p-4"><p className="text-xs text-[var(--text-muted)]">Transactions</p><p className="mt-1 text-xl font-black">{transactions.length}</p></div></div>
   </Surface>

   <Surface className="overflow-hidden">
    <div className="border-b border-[var(--border)] px-4 py-3.5 sm:px-5"><h2 className="text-sm font-black">By service</h2></div>
    {serviceEarnings.length?<div className="grid divide-y divide-[var(--border)] sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">{serviceEarnings.map(x=><Link href={"/transactions?type="+x.kind} key={x.kind} className="p-4 transition hover:bg-[var(--surface-soft)]"><div className="flex items-start justify-between gap-2"><p className="text-sm font-bold">{serviceName(x.kind)}</p><span className="text-[10px] font-semibold text-[var(--text-muted)]">{x.count} tx</span></div><p className="money mt-2 text-xl font-black">{money(x.gross)}</p><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px]"><span className="font-bold text-[var(--money-in)]">Earned {money(x.commission)}</span><span className="text-[var(--money-out)]">Fees {money(x.charges)}</span></div></Link>)}</div>:<div className="p-5"><EmptyState title="No service activity"/></div>}
   </Surface>
