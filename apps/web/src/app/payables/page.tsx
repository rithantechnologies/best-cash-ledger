...
type Payable={id:string;originalAmount:string;paidAmount:string;remainingAmount:string;dueAt:string;status:string;customer:{fullName:string}};
...
export default function PayablesPage(){
 ...
 useEffect(()=>{const timer=setTimeout(()=>load(1).catch(()=>setError("Failed to load payables")),180);return()=>clearTimeout(timer);},[q,status,sortBy,sortDir,pagination.pageSize]);
 ...
 return <AppShell><PageFrame>
  ...
  {loading?<PageLoader label="Loading payables…"/>:<>
   <div className="space-y-2 md:hidden">{items.map(p=><Surface key={p.id} className="p-4">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-bold">{p.customer.fullName}</p><p className="mt-0.5 text-[11px] text-slate-400">Due {new Date(p.dueAt).toLocaleDateString("en-IN")}</p></div><StatusBadge tone={statusTone(p.status) as "slate"|"emerald"|"indigo"|"amber"|"rose"}>{p.status.replaceAll("_"," ")}</StatusBadge></div>
    ...
    <div className="mt-3 grid grid-cols-2 gap-2"><Link href={"/payables/"+p.id} className="flex min-h-10 items-center justify-center rounded-xl border border-slate-200 text-xs font-bold">View details</Link><button onClick={()=>{setSelected(p);setAmount(p.remainingAmount);setSource("");}} disabled={Number(p.remainingAmount)<=0||["CANCELLED","REVERSED","PAID"].includes(p.status)} className="min-h-10 rounded-xl bg-slate-950 text-xs font-bold text-white disabled:opacity-40">Pay now</button></div>
