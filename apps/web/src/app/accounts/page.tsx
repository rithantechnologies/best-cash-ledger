...
export default function AccountsPage(){
  ...
  return <AppShell><PageFrame width="max-w-7xl" className="ui-preview accounts-preview">
    ...
    <div className="grid grid-cols-3 gap-2.5">
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-[3px] bg-[var(--accent)]"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[var(--accent)]">Funds</p><p className="money mt-1 text-lg font-bold sm:text-2xl">{money(totals.liquid)}</p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text-muted)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 7h16v11H4z"/><path d="M16 11h4v4h-4a2 2 0 0 1 0-4Z"/></svg></span></div></Surface>
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-[3px] bg-rose-500"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-rose-600">Card due</p><p className="money mt-1 text-lg font-bold text-rose-600 sm:text-2xl">{money(totals.cards)}</p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text-muted)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18"/></svg></span></div></Surface>
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-[3px] bg-emerald-500"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-emerald-700">Active</p><p className="mt-1 text-lg font-bold sm:text-2xl">{totals.active}<span className="ml-1 text-xs font-medium text-[var(--text-muted)]">/ {visibleItems.length}</span></p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text-muted)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="m7 12 3 3 7-7"/><circle cx="12" cy="12" r="9"/></svg></span></div></Surface>
    </div>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        ...
        <div className="ui-scroll-fade min-w-0 lg:flex-none"><div className="flex gap-1.5 overflow-x-auto pb-0.5 pr-5 lg:pr-0">
          {typeTabs.map(([value,label])=><button type="button" key={value} onClick={()=>setListType(value)}
            className={"min-h-11 shrink-0 rounded-xl px-3 text-xs font-semibold transition "+(listType===value?"bg-[var(--accent)] text-white":"border border-[var(--border)] bg-[var(--surface)] text-[var(--text-muted)] hover:text-[var(--text)]")}>
            {label}<span className={"ml-1.5 "+(listType===value?"text-white/75":"text-[var(--text-muted)]")}>{counts[value]??0}</span>
          </button>)}
        </div></div>
      </div>
    </Surface>

    {filteredItems.length?<div className="grid grid-cols-2 gap-2.5 lg:grid-cols-3 xl:grid-cols-4">
      {filteredItems.map((account)=>{
        ...
        return <Surface key={account.id} className={"group flex min-h-[172px] flex-col p-3 transition hover:border-[color-mix(in_srgb,var(--accent)_22%,var(--border))] sm:min-h-[184px] sm:p-4 "+(!account.isActive?"opacity-60":"")}>
          <div className="flex items-start gap-2.5">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text-muted)]"><AccountIcon type={account.accountType}/></span>
            <div className="min-w-0 flex-1">
              <p className="line-clamp-2 text-[13px] font-semibold leading-[1.3] tracking-[-.01em] sm:text-sm">{account.accountName}</p>
              <div className="mt-1 flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--accent)]"/>
                <span className="truncate text-[10px] font-semibold uppercase tracking-[.07em] text-[var(--text-muted)]">{typeLabels[account.accountType]??account.accountType}</span>
              </div>
            </div>
            {!account.isActive?<span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-slate-500">Off</span>:null}
          </div>
          <div className="mt-3">
            <p className="text-[10px] font-medium uppercase tracking-[.07em] text-[var(--text-muted)]">{account.accountType==="OWNER_CREDIT_CARD"?"Outstanding":"Balance"}</p>
            <p className={"money mt-1 text-xl font-bold tracking-[-.035em] sm:text-[1.45rem] "+(account.accountType==="OWNER_CREDIT_CARD"&&account.currentBalance>0?"text-rose-600":"")}>{money(account.currentBalance)}</p>
            {meta?<p className="mt-1 truncate text-[11px] text-[var(--text-muted)]">{meta}</p>:null}
          </div>
          {account.accountType==="OWNER_CREDIT_CARD"&&account.creditLimit!==null?<div className="mt-3">
            <div className="h-1.5 overflow-hidden rounded-full bg-[var(--surface-soft)]"><div className="h-full rounded-full bg-[var(--accent)]" style={{width:cardUsed+"%"}}/></div>
            <div className="mt-1.5 flex justify-between gap-2 text-[10px] text-[var(--text-muted)]"><span>{cardUsed.toFixed(0)}% used</span><span>{money(account.availableCredit??0)} free</span></div>
          </div>:null}
          <div className={"mt-auto grid gap-2 border-t border-[var(--border)] pt-3 "+(admin&&account.accountType!=="PROVIDER_WALLET"?"grid-cols-2":"grid-cols-1")}>
            <Link href={"/accounts/"+account.id} className="flex min-h-11 items-center justify-center rounded-xl bg-[var(--accent-soft)] px-3 text-xs font-semibold text-[var(--accent)]">Ledger</Link>
            {admin&&account.accountType!=="PROVIDER_WALLET"?<button type="button" title={account.isActive?"Edit account":"Reactivate account"} onClick={(event)=>{event.preventDefault();event.stopPropagation();setToggleTarget(null);if(account.isActive)beginEdit(account);else setToggleTarget(account);}} className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-semibold text-[var(--text)]" aria-label={account.isActive?"Edit "+account.accountName:"Reactivate "+account.accountName}>
              {account.isActive?<><svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 20h4l11-11a2.8 2.8 0 0 0-4-4L4 16v4Z"/><path d="m13.5 6.5 4 4"/></svg>Edit</>:<>↻ Reactivate</>}
            </button>:null}
          </div>
        </Surface>;
      })}
