...
function accountTone(type:string){
  if(type==="CASH")return {bar:"bg-emerald-500"};
  if(type==="UPI")return {bar:"bg-violet-500"};
  if(type==="PROVIDER_WALLET")return {bar:"bg-amber-500"};
  if(type==="OWNER_CREDIT_CARD")return {bar:"bg-rose-500"};
...
export default function AccountsPage(){
  ...
  return <AppShell><PageFrame width="max-w-7xl">
    ...
    <div className="grid grid-cols-3 gap-2.5">
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-1 bg-indigo-500"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-indigo-600">Funds</p><p className="money mt-1 text-lg font-black sm:text-2xl">{money(totals.liquid)}</p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[var(--text)] text-[var(--surface)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 7h16v11H4z"/><path d="M16 11h4v4h-4a2 2 0 0 1 0-4Z"/></svg></span></div></Surface>
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-1 bg-rose-500"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-rose-600">Card due</p><p className="money mt-1 text-lg font-black text-rose-600 sm:text-2xl">{money(totals.cards)}</p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[var(--text)] text-[var(--surface)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18"/></svg></span></div></Surface>
      <Surface className="relative overflow-hidden p-3.5 sm:p-4"><span className="absolute inset-x-0 top-0 h-1 bg-emerald-500"/><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.08em] text-emerald-700">Active</p><p className="mt-1 text-lg font-black sm:text-2xl">{totals.active}<span className="ml-1 text-xs font-semibold text-[var(--text-muted)]">/ {items.length}</span></p></div><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[var(--text)] text-[var(--surface)]"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="m7 12 3 3 7-7"/><circle cx="12" cy="12" r="9"/></svg></span></div></Surface>
    </div>
    ...
    {filteredItems.length?<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
      {filteredItems.map((account)=>{
        ...
        return <Surface key={account.id} className={"group relative flex min-h-[198px] flex-col overflow-hidden border border-[var(--border)] p-4 shadow-[0_8px_24px_rgba(15,23,42,.045)] transition hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--accent)_22%,var(--border))] hover:shadow-[0_16px_36px_rgba(15,23,42,.08)] "+(!account.isActive?"opacity-60":"")}>
          <span className={"absolute inset-y-0 left-0 w-[3px] "+tone.bar}/>
          <div className="relative flex items-start gap-3 pt-1">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text)] shadow-sm"><AccountIcon type={account.accountType}/></span>
            ...
            <span className="rounded-full border border-[var(--border)] bg-[var(--surface-soft)] px-2.5 py-1 text-[10px] font-bold capitalize text-[var(--text-muted)]">{account.usageType.toLowerCase()}</span>
          </div>
          ...
          <div className="relative z-10 mt-auto grid grid-cols-2 gap-2 border-t border-[var(--border)] pt-3">
            <Link href={"/accounts/"+account.id} className="flex min-h-10 items-center justify-center rounded-xl bg-[var(--accent-soft)] px-3 text-xs font-extrabold text-[var(--accent)] transition hover:brightness-[.98]">Ledger <span className="ml-1">→</span></Link>
            {admin&&account.accountType!=="PROVIDER_WALLET"?<button type="button" onClick={(event)=>{event.preventDefault();event.stopPropagation();setToggleTarget(null);if(account.isActive)beginEdit(account);else setToggleTarget(account);}} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-extrabold text-[var(--text)] transition hover:border-[var(--accent)] hover:text-[var(--accent)]">
            ...
            </button>:<Link href={"/accounts/"+account.id} className="flex min-h-10 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-xs font-extrabold text-[var(--text-muted)]">Details</Link>}
          </div>
        </Surface>;
      })}
