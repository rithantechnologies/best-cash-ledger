...
const commissionCashPart=(total:number,mode?:string|null,cash?:number|string|null)=>{
  ...
  return mode==="CASH"?Number(total||0):0;
};
const commissionDigitalPart=(total:number,mode?:string|null,cash?:number|string|null)=>
  Math.max(0,Number(total||0)-commissionCashPart(total,mode,cash));
const commissionDigitalLabel=(accountType?:string|null)=>{
  if(accountType==="BANK")return "Bank";
  if(accountType==="UPI")return "UPI";
  return "Bank/UPI";
};
const mirrorsCompletionCommissionAccount=(item?:QuickCashPending|null,beneficiaryMode?:"UPI"|"BANK")=>Boolean(
  item&&commissionDigitalPart(Number(item.commissionAmount||0),item.commissionMode,item.commissionCashAmount)>0&&(
    (item.direction==="OUT"&&(item.cashOutType??"UPI_QR")==="UPI_QR")||
    (item.direction==="IN"&&(beneficiaryMode??item.beneficiaryMode??"UPI")==="UPI")
  ),
);
const commissionBreakdownLabel=(total:number,mode?:string|null,cash?:number|string|null)=>{
  ...
  if(cashAmount>0&&digitalAmount>0)return money(cashAmount)+" Cash + "+money(digitalAmount)+" Bank/UPI";
  if(cashAmount>0)return "Cash";
  if(digitalAmount>0)return "Bank/UPI";
  return "";
};
...
const commissionReceiptLabel=(activity:Activity)=>{
  ...
  if(!total)return "";
  const cash=commissionCashPart(total,activity.commissionMode,activity.commissionCashAmount);
  const digital=commissionDigitalPart(total,activity.commissionMode,activity.commissionCashAmount);
  if(cash>0&&digital>0)return money(cash)+" Cash · "+money(digital)+" "+commissionDigitalLabel(activity.commissionAccountType);
...
export default function CashCounterPage(){
  ...
  const [quickSuccessful,setQuickSuccessful]=useState(true);
  ...
  const completeAllocationValid=!bankTransferSplitCompletion||(
    completeSourceAllocations.length>0&&
  ...
  const cashBookRows=useMemo(()=>{
    const activities=[...(today?.activities??[])]
      .filter((activity)=>activity.serviceType!=="REVERSAL"&&activity.transactionStatus!=="REVERSED")
      .reverse();
  ...
  async function saveQuickCash(event:FormEvent){
    ...
    if(purpose==="TRANSFER"&&!(quickDirection==="OUT"&&quickCashOutType==="MICRO_ATM")&&quickCommissionMode==="SPLIT"&&(!Number.isFinite(commissionCashAmount)||commissionCashAmount<=0||commissionCashAmount>=commissionAmount))validation.commission="For split commission, enter a cash part greater than 0 and less than the total commission.";
    ...
    try{
      ...
      await apiFetch("/transactions/quick-cash",{method:"POST",body:JSON.stringify({
        ...
        commissionCashAmount:purpose==="TRANSFER"&&commissionAmount>0&&!(quickDirection==="OUT"&&quickCashOutType!=="UPI_QR"&&!quickSuccessful)?(quickDirection==="OUT"&&quickCashOutType==="MICRO_ATM"?0:commissionCashAmount):undefined,
        commissionMode:purpose==="TRANSFER"&&commissionAmount>0&&!(quickDirection==="OUT"&&quickCashOutType!=="UPI_QR"&&!quickSuccessful)?(quickDirection==="OUT"&&quickCashOutType==="MICRO_ATM"?"UPI":quickCommissionMode):undefined,
  ...
  async function completeQuickCash(event:FormEvent){
    ...
    try{
      ...
      await apiFetch("/transactions/quick-cash/"+completePendingId+"/complete",{method:"POST",body:JSON.stringify({
        sourceAccountId:bankTransferSplitCompletion?undefined:completeSourceAccountId,
        sourceAllocations:bankTransferSplitCompletion?completeSourceAllocations.map((row)=>({sourceAccountId:row.sourceAccountId,amount:Number(row.amount),referenceNumber:row.referenceNumber.trim()||undefined})):undefined,
        commissionAccountId:commissionDigitalPart(Number(completingQuickCash?.commissionAmount||0),completingQuickCash?.commissionMode,completingQuickCash?.commissionCashAmount)>0?completeCommissionAccountId||undefined:undefined,
  ...
  function openPendingCompletion(item:QuickCashPending){
    ...
    const defaultAllocations=item.direction==="IN"&&mode==="BANK"
      ?[{sourceAccountId:paySwitchCompletionAccountId,amount:String(Number(item.amount||0)),referenceNumber:""}]
      :[];
    const defaultCommissionAccountId=mirrorsCompletionCommissionAccount(item,mode)
      ?defaultSourceAccountId
      :item.direction==="IN"&&mode==="BANK"&&defaultAllocations.length===1
        ?defaultAllocations[0].sourceAccountId
        :"";
    setCompletePendingId(item.id);setCompleteSourceAccountId(defaultSourceAccountId);setCompleteSourceAllocations(defaultAllocations);setCompleteCommissionAccountId(defaultCommissionAccountId);setCompleteCommissionAccountOverridden(false);
    ...
    {portalReady&&quickDirection?createPortal(<div className="fixed inset-0 z-[100] grid place-items-end bg-black/45 p-0 sm:place-items-center sm:p-5" role="dialog" aria-modal="true">
      <form onSubmit={saveQuickCash} className="cash-quick-sheet flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-[28px] bg-[var(--surface)] shadow-2xl sm:max-h-[86dvh] sm:max-w-[520px] sm:rounded-[26px]">
        ...
        <div className="px-5 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4">
          ...
          {quickDirection==="OUT"&&quickCashOutType!=="UPI_QR"?<div className="mt-3 rounded-[17px] bg-[var(--surface-soft)] p-3">
            <div className="grid grid-cols-2 rounded-xl bg-[var(--surface)] p-1">
              ...
              <button type="button" onClick={()=>{setQuickSuccessful(false);setQuickCommission("");setQuickCommissionCash("");setQuickError("");clearQuickFieldError("commission");}} className={"min-h-9 rounded-lg text-[12px] font-black "+(!quickSuccessful?"bg-rose-50 text-rose-700 shadow-sm":"text-[var(--text-muted)]")}>Failed attempt</button>
            </div>
        ...
        <footer className="shrink-0 border-t border-[var(--border)] bg-[var(--surface)] px-5 py-3 pb-[max(.75rem,env(safe-area-inset-bottom))]">
          <button disabled={quickSaving} className={"min-h-[52px] w-full rounded-[16px] px-5 text-base font-black text-white shadow-[0_10px_22px_rgba(15,23,42,.12)] transition active:scale-[.99] disabled:opacity-50 "+(quickDirection==="IN"?"bg-emerald-600":"bg-rose-600")}>{quickSaving?"Saving…":quickPurpose==="SERVICE"?"Record service":quickDirection==="OUT"&&quickCashOutType!=="UPI_QR"&&!quickSuccessful?"Record failed attempt":quickDirection==="OUT"?"Save cash out":"Save transfer"}</button>
        </footer>
      </form>
    </div>,document.body):null}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
          ...
          <div className={"grid gap-3 "+(commissionDigitalPart(Number(completingQuickCash?.commissionAmount||0),completingQuickCash?.commissionMode,completingQuickCash?.commissionCashAmount)>0?"lg:grid-cols-2":"")}>
            <div className="block rounded-[20px] border border-[var(--border)] bg-[var(--surface-soft)] p-4 transition focus-within:border-[var(--accent)] focus-within:ring-2 focus-within:ring-[var(--accent-soft)]">
              ...
              {bankTransferSplitCompletion?<div className="mt-3 space-y-2">
                {completeSourceAllocations.map((row,index)=><div key={index} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3">
                  <div className="grid gap-2 sm:grid-cols-[minmax(0,1.25fr)_130px_minmax(0,1fr)_auto] sm:items-end">
                    <div>
                      <span className="mb-1 block text-[9px] font-black uppercase tracking-[.08em] text-[var(--text-muted)]">Wallet</span>
                      <SearchableSelect mobileSheet aria-label={"Wallet source "+(index+1)} searchPlaceholder="Search wallet" className="min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2.5 text-sm font-black text-[var(--text)] outline-none" value={row.sourceAccountId} onChange={(event)=>{const value=event.target.value;setCompleteSourceAllocations((current)=>current.map((item,rowIndex)=>rowIndex===index?{...item,sourceAccountId:value}:item));if(completeSourceAllocations.length===1&&!completeCommissionAccountOverridden)setCompleteCommissionAccountId(value);setCompleteError("");}}>
                        <option value="">Select wallet</option>
                        {completionWalletAccounts.map((account)=><option key={account.id} value={account.id}>{account.accountName} · {money(account.currentBalance??0)}</option>)}
                      </SearchableSelect>
                    </div>
                    <label>
                      <span className="mb-1 block text-[9px] font-black uppercase tracking-[.08em] text-[var(--text-muted)]">Amount</span>
                      <input inputMode="decimal" className="min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2.5 text-right text-sm font-black outline-none" value={row.amount} onChange={(event)=>{const value=event.target.value.replace(/[^0-9.]/g,"");setCompleteSourceAllocations((current)=>current.map((item,rowIndex)=>rowIndex===index?{...item,amount:value}:item));setCompleteError("");}} placeholder="0"/>
                    </label>
                    <label>
                      <span className="mb-1 block text-[9px] font-black uppercase tracking-[.08em] text-[var(--text-muted)]">Reference / UTR <span className="normal-case font-semibold">(optional)</span></span>
                      <input className="min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2.5 text-sm font-bold outline-none" value={row.referenceNumber} onChange={(event)=>setCompleteSourceAllocations((current)=>current.map((item,rowIndex)=>rowIndex===index?{...item,referenceNumber:event.target.value}:item))} placeholder="UTR"/>
                    </label>
                    <button type="button" disabled={completeSourceAllocations.length===1} onClick={()=>setCompleteSourceAllocations((current)=>{const next=current.filter((_,rowIndex)=>rowIndex!==index);if(next.length===1&&!completeCommissionAccountOverridden)setCompleteCommissionAccountId(next[0].sourceAccountId);return next;})} className="min-h-11 rounded-lg px-3 text-xs font-black text-rose-600 disabled:opacity-30">Remove</button>
                  </div>
                </div>)}
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                  <button type="button" onClick={()=>{setCompleteSourceAllocations((current)=>[...current,{sourceAccountId:"",amount:completeAllocationRemaining>0?String(Math.round(completeAllocationRemaining*100)/100):"",referenceNumber:""}]);if(!completeCommissionAccountOverridden)setCompleteCommissionAccountId("");}} className="min-h-10 rounded-xl border border-dashed border-[var(--accent)] px-3 text-xs font-black text-[var(--accent)]">+ Add another wallet</button>
            ...
            {commissionDigitalPart(Number(completingQuickCash?.commissionAmount||0),completingQuickCash?.commissionMode,completingQuickCash?.commissionCashAmount)>0?<label className="block rounded-[20px] border border-blue-200 bg-blue-50/55 p-4 transition focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100">
              ...
              <SearchableSelect mobileSheet aria-label="Commission received in" searchPlaceholder="Search bank / UPI account" className="mt-3 min-h-12 w-full rounded-xl border border-blue-200 bg-[var(--surface)] px-3 text-[16px] font-black text-[var(--text)] outline-none" value={completeCommissionAccountId} onChange={(event)=>{const value=event.target.value;const linkedSource=bankTransferSplitCompletion&&completeSourceAllocations.length===1?completeSourceAllocations[0].sourceAccountId:completeSourceAccountId;setCompleteCommissionAccountId(value);setCompleteCommissionAccountOverridden(Boolean(value&&value!==linkedSource));setCompleteError("");}}>
