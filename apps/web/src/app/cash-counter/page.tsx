      ...
      {pendingQuickCash.length?<Surface className="scroll-mt-24 overflow-hidden">
        ...
        <div className="divide-y divide-[var(--border)]">
          {pendingQuickCash.map((item)=><div key={item.id} className="grid gap-3 px-4 py-3.5 sm:grid-cols-[110px_minmax(0,1fr)_auto_auto] sm:items-center sm:px-5">
            ...
            <div className="text-left sm:text-right"><strong className="money block text-sm">{money(item.amount)}</strong>{Number(item.commissionAmount)>0?<span className="text-xs font-semibold text-[var(--accent)]">{item.cashOutType==="AEPS"||item.cashOutType==="MICRO_ATM"?"Commission "+money(item.commissionAmount)+" · settle later":"Fee "+money(item.commissionAmount)+" · "+commissionBreakdownLabel(Number(item.commissionAmount),item.commissionMode,item.commissionCashAmount)}</span>:null}</div>
        ...
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--border)] px-5 py-4 sm:px-6 sm:py-5">
          <div className="min-w-0">
            ...
            {completingQuickCash?<div className="mt-2 flex flex-wrap gap-2 text-xs font-bold">
              ...
              <span className="rounded-full bg-[var(--surface-soft)] px-2.5 py-1">{Number(completingQuickCash.commissionAmount)>0?(completingQuickCash.cashOutType==="AEPS"||completingQuickCash.cashOutType==="MICRO_ATM"?"Commission "+money(completingQuickCash.commissionAmount)+" · settlement pending":"Commission "+money(completingQuickCash.commissionAmount)+" · "+commissionBreakdownLabel(Number(completingQuickCash.commissionAmount),completingQuickCash.commissionMode,completingQuickCash.commissionCashAmount)):"No commission"}</span>
