 ...
 return <AppShell><PageFrame>
  ...
  <Modal open={!!edit} title={edit?"Edit "+edit.kind.replace("beneficiaryAccount","recipient account"):"Edit saved detail"} description="Update saved details without changing historical transactions." onClose={()=>setEdit(null)} footer={<button form="edit-saved-detail" disabled={busy} className="min-h-11 w-full rounded-xl bg-indigo-700 font-bold text-white">Save changes</button>}>
   <form id="edit-saved-detail" onSubmit={saveEdit} className="grid gap-3 sm:grid-cols-2">
    {edit?.kind==="card"?<><Field label="Bank"><input className={control} value={e1} onChange={e=>setE1(e.target.value)} required/></Field><Field label="Last 4"><input className={control} value={e2} onChange={e=>setE2(e.target.value.replace(/\D/g,"").slice(0,4))} maxLength={4} required/></Field><Field label="Type"><SearchableSelect className={control} value={e3} onChange={e=>setE3(e.target.value)}>{["CREDIT","DEBIT","BUSINESS","OTHER"].map(x=><option key={x}>{x}</option>)}</SearchableSelect></Field><Field label="Nickname"><input className={control} value={e4} onChange={e=>setE4(e.target.value)}/></Field></>:null}
